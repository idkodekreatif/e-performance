...
namespace App\Http\Controllers\iktisar\iktisarBulanan\keuangan;

use App\Http\Controllers\Controller;
use App\Models\iktisar\staff\keuangan\iktisarKeuanganBulananStaffKompetensi;
use App\Models\iktisar\staff\keuangan\iktisarKeuanganBulananStaffPerilaku;
use App\Models\User;
use Carbon\Carbon;
...
class iktisarBulananKeuanganStaffController extends Controller
{
    // penilai
    public function create()
    {
        ...
        return view('iktisar.keuangan.iktisarBulananStaff.create', compact('users'));
    }
    // preses penilai
    public function store(Request $request)
    {
        DB::beginTransaction();
        try {
            // Insert data into iktisar_keuangan_bulanan_perilaku table
            $data1 = [
            ...
            // Insert the data into the iktisar_keuangan_bulanan_perilaku table and get the inserted row ID
            $iktisarStaffBulananPerilaku = iktisarKeuanganBulananStaffPerilaku::create($data1);
            $id1 = $iktisarStaffBulananPerilaku->id;

            // Insert data into iktisar_keuangan_bulanan_kompetensi table for each job type in the jenisPekerjaan array
            function saveData($jenisPekerjaan, $jumlahBobot, $question, $id1)
            {
                foreach ($jenisPekerjaan as $key => $value) {
                    ...
                    iktisarKeuanganBulananStaffKompetensi::create($data2);
                }
            }
    ...
    public function searchDataEdit()
    {
        ...
        return view('iktisar.keuangan.iktisarBulananStaff.searchdata', compact('users'));
    }
    // penilai
    public function edit(Request $request)
    {
        ...
        $iktisarStaffBulananPerilaku = DB::table('iktisar_keuangan_bulanan_perilaku')
        ...
        if ($iktisarStaffBulananPerilaku) {
            // Retrieve the corresponding rows from the iktisar_keuangan_bulanan_kompetensi table
            $iktisarStaffBulananKompetensi = DB::table('iktisar_keuangan_bulanan_kompetensi')
        ...
        return view('iktisar.keuangan.iktisarBulananStaff.edit', [
            'users' => $users,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarKeuanganBulananStaffPerilaku::find($id);
            ...
            $existingJenisPekerjaan = iktisarKeuanganBulananStaffKompetensi::where('id_staff_perilaku', $id)->pluck('jenis_pekerjaan')->toArray();

            // Loop melalui jenis pekerjaan yang diterima dari form
            foreach ($jenisPekerjaan as $key => $value) {
                $iktisarStaffBulananKompetensi = iktisarKeuanganBulananStaffKompetensi::where('id_staff_perilaku', $id)
                ...
                if (!$iktisarStaffBulananKompetensi) {
                    ...
                    iktisarKeuanganBulananStaffKompetensi::create($data);
                } else {
                    ...
                    iktisarKeuanganBulananStaffKompetensi::where('id', $iktisarStaffBulananKompetensi->id)->update($data);
                }
            }
            ...
            foreach ($existingJenisPekerjaan as $existingJenis) {
                ...
                if (!in_array($existingJenis, $jenisPekerjaan)) {
                    iktisarKeuanganBulananStaffKompetensi::where('id_staff_perilaku', $id)->where('jenis_pekerjaan', $existingJenis)->delete();
                }
            }
    ...
    public function raportStaff(Request $request, $user_id)
    {
        ...
        if ($request->filled('keyword')) {
            ...
            $users = $dataUser->whereExists(function ($query) use ($user_id, $date) {
                $query->select(DB::raw(1))
                    ->from('iktisar_keuangan_bulanan_perilaku')
                    ->whereRaw('users.id = iktisar_keuangan_bulanan_perilaku.user_id')
                    ->whereMonth('iktisar_keuangan_bulanan_perilaku.created_insert', $date->month)
                    ->whereYear('iktisar_keuangan_bulanan_perilaku.created_insert', $date->year)
                    ->where('iktisar_keuangan_bulanan_perilaku.user_id', $user_id);
            });
        } else {
            $users = $dataUser;
        }
        ...
        $data = $users
            ->leftJoin('iktisar_keuangan_bulanan_perilaku', 'users.id', '=', 'iktisar_keuangan_bulanan_perilaku.user_id')
            ->select(
                'users.*',
                'iktisar_keuangan_bulanan_perilaku.user_id',
                'iktisar_keuangan_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'iktisar_keuangan_bulanan_perilaku.total_nilai_presentase',
            )
            ->where('iktisar_keuangan_bulanan_perilaku.user_id', $user_id)
            ->whereMonth('iktisar_keuangan_bulanan_perilaku.created_insert', $date->month)
            ->whereYear('iktisar_keuangan_bulanan_perilaku.created_insert', $date->year)
        ...
        if (!empty($data->user_id)) {
            return view('iktisar.keuangan.iktisarBulananStaff.raport', compact('data'));
        } else {
            toast('Data Empty :(', 'error');
            return redirect()->back();
        }
    ...
    {
        return view('iktisar.keuangan.iktisarBulananStaff.searchdatapoin');
    }
    ...
    public function dataPoin(Request $request)
    {
        ...
        // Display the data on a form
        return view('iktisar.keuangan.iktisarBulananStaff.detailPoin', [
            'users' => $users,
            'iktisarStaffBulananPerilaku' => $iktisarStaffBulananPerilaku,
            'iktisarStaffBulananKompetensi' => $iktisarStaffBulananKompetensi
        ]);
    ...
    public function searchRaportIktisar()
    {
        ...
        return view('iktisar.keuangan.iktisarBulananStaff.searchdataraport', compact('users'));
    }
    ...
    //             'iktisar_keuangan_bulanan_perilaku.total_nilai_presentase',
    //         )
    //         ->where('iktisar_keuangan_bulanan_perilaku.user_id', $id)
    //         ->whereYear('iktisar_keuangan_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->whereMonth('iktisar_keuangan_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->first();

    //     // dd($data);
    //     if (!empty($data)) {
    //         return view('iktisar.keuangan.iktisarBulananStaff.cekraport', compact('data'));
    //     } else {
    //         toast('Data Empty', 'error');
    //         return redirect()->back();
    //     }
    ...
    public function staffRaportIktisar(Request $request)
    {
        ...
        if (!empty($data)) {
            if ($request->input('type') === 'pdf') {
                $pdf = PDF::loadView('iktisar.keuangan.iktisarBulananStaff.raportPdf', compact('data', 'tanggalInput'))->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'potrait');
                return $pdf->download('raport_' . $data->name . '_' . $tanggalInput->format('Y-m-d') . '.pdf');
            } else {
                return view('iktisar.keuangan.iktisarBulananStaff.cekraport', compact('data', 'tanggalInput'));
            }
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
    ...
    public function rekap()
    {
        ...
        return view('iktisar.keuangan.iktisarBulananStaff.rekap.index', compact('users'));
    }
    ...
    public function reportRaport(Request $request)
    {
        ...
        $data = DB::table('users')
            ...
            ->select(
                ...
                'iktisar_keuangan_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'iktisar_keuangan_bulanan_perilaku.total_nilai_presentase',
                'iktisar_keuangan_bulanan_perilaku.created_insert'
            )
            ->where('iktisar_keuangan_bulanan_perilaku.user_id', $id)
            ->orderBy('iktisar_keuangan_bulanan_perilaku.created_insert', 'desc') // Urutkan dari terbaru ke lama
            ->get(); // Menggunakan get() agar mendapatkan semua data
        // dd($data);

        if ($data->isNotEmpty()) {
            return view('iktisar.keuangan.iktisarBulananStaff.rekap.report', compact('data'));
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
        }
    ...
    {
        // Ambil data dari tabel iktisar_keuangan_bulanan_perilaku
        $iktisarStaffBulananPerilaku = DB::table('iktisar_keuangan_bulanan_perilaku')
        ...
        // Ambil data dari tabel iktisar_keuangan_bulanan_kompetensi berdasarkan id_staff_perilaku
        $iktisarStaffBulananKompetensi = DB::table('iktisar_keuangan_bulanan_kompetensi')
            ->where('id_staff_perilaku', $iktisarStaffBulananPerilaku->id)
            ->get();

        // Kirim data ke view
        return view('iktisar.keuangan.iktisarBulananStaff.rekap.show', compact('iktisarStaffBulananPerilaku', 'iktisarStaffBulananKompetensi'));
    }
}
