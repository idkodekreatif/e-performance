...
namespace App\Http\Controllers\iktisar\iktisarBulanan\rektor;

use App\Http\Controllers\Controller;
use App\Models\iktisar\kaunit\rektor\iktisarRektorBulananKompetensi;
use App\Models\iktisar\kaunit\rektor\iktisarRektorBulananPerilaku;
use App\Models\User;
use Carbon\Carbon;
...
class iktisarBulananRektorController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('iktisar.rektor.iktisarBulananKaUnit.create', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarRektorBulananPerilaku::create($data1);
            ...
            function saveData($jenisPekerjaan, $jumlahBobot, $question, $id1)
            {
                foreach ($jenisPekerjaan as $key => $value) {
                    ...
                    iktisarRektorBulananKompetensi::create($data2);
                }
            }
    ...
    public function searchDataEdit()
    {
        ...
        return view('iktisar.rektor.iktisarBulananKaUnit.searchdata', compact('users'));
    }
    // penilai
    public function edit(Request $request)
    {
        ...
        $iktisarStaffBulananPerilaku = DB::table('iktisar_rektor_bulanan_perilaku')
        ...
        if ($iktisarStaffBulananPerilaku) {
            // Retrieve the corresponding rows from the iktisar_rektor_bulanan_kompetensi table
            $iktisarStaffBulananKompetensi = DB::table('iktisar_rektor_bulanan_kompetensi')
        ...
        return view('iktisar.rektor.iktisarBulananKaUnit.edit', [
            'users' => $users,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarRektorBulananPerilaku::find($id);
            ...
            $existingJenisPekerjaan = iktisarRektorBulananKompetensi::where('id_staff_perilaku', $id)->pluck('jenis_pekerjaan')->toArray();

            // Loop melalui jenis pekerjaan yang diterima dari form
            foreach ($jenisPekerjaan as $key => $value) {
                $iktisarStaffBulananKompetensi = iktisarRektorBulananKompetensi::where('id_staff_perilaku', $id)
                ...
                if (!$iktisarStaffBulananKompetensi) {
                    ...
                    iktisarRektorBulananKompetensi::create($data);
                } else {
                    ...
                    iktisarRektorBulananKompetensi::where('id', $iktisarStaffBulananKompetensi->id)->update($data);
                }
            }
            ...
            foreach ($existingJenisPekerjaan as $existingJenis) {
                ...
                if (!in_array($existingJenis, $jenisPekerjaan)) {
                    iktisarRektorBulananKompetensi::where('id_staff_perilaku', $id)->where('jenis_pekerjaan', $existingJenis)->delete();
                }
            }
    ...
    public function raportStaff(Request $request, $user_id)
    {
        ...
        if ($request->filled('keyword')) {
            ...
            $users = $dataUser->whereExists(function ($query) use ($user_id, $date) {
                $query->select(DB::raw(1))
                    ->from('iktisar_rektor_bulanan_perilaku')
                    ->whereRaw('users.id = iktisar_rektor_bulanan_perilaku.user_id')
                    ->whereMonth('iktisar_rektor_bulanan_perilaku.created_insert', $date->month)
                    ->whereYear('iktisar_rektor_bulanan_perilaku.created_insert', $date->year)
                    ->where('iktisar_rektor_bulanan_perilaku.user_id', $user_id);
            });
        } else {
            $users = $dataUser;
        }
        ...
        $data = $users
            ->leftJoin('iktisar_rektor_bulanan_perilaku', 'users.id', '=', 'iktisar_rektor_bulanan_perilaku.user_id')
            ->select(
                'users.*',
                'iktisar_rektor_bulanan_perilaku.user_id',
                'iktisar_rektor_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'iktisar_rektor_bulanan_perilaku.total_nilai_presentase',
            )
            ->where('iktisar_rektor_bulanan_perilaku.user_id', $user_id)
            ->whereMonth('iktisar_rektor_bulanan_perilaku.created_insert', $date->month)
            ->whereYear('iktisar_rektor_bulanan_perilaku.created_insert', $date->year)
        ...
        if (!empty($data->user_id)) {
            return view('iktisar.rektor.iktisarBulananKaUnit.raport', compact('data'));
        } else {
            toast('Data Empty :(', 'error');
            return redirect()->back();
        }
    ...
    {
        return view('iktisar.rektor.iktisarBulananKaUnit.searchdatapoin');
    }
    ...
    public function dataPoin(Request $request)
    {
        ...
        // Display the data on a form
        return view('iktisar.rektor.iktisarBulananKaUnit.detailPoin', [
            'users' => $users,
            'iktisarStaffBulananPerilaku' => $iktisarStaffBulananPerilaku,
            'iktisarStaffBulananKompetensi' => $iktisarStaffBulananKompetensi
        ]);
    ...
    public function searchRaportIktisar()
    {
        ...
        return view('iktisar.rektor.iktisarBulananKaUnit.searchdataraport', compact('users'));
    }
    ...
    //             'iktisar_rektor_bulanan_perilaku.total_nilai_presentase',
    //         )
    //         ->where('iktisar_rektor_bulanan_perilaku.user_id', $id)
    //         ->whereYear('iktisar_rektor_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->whereMonth('iktisar_rektor_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->first();

    //     // dd($data);
    //     if (!empty($data)) {
    //         return view('iktisar.rektor.iktisarBulananKaUnit.cekraport', compact('data'));
    //     } else {
    //         toast('Data Empty', 'error');
    //         return redirect()->back();
    //     }
    ...
    public function staffRaportIktisar(Request $request)
    {
        ...
        if (!empty($data)) {
            if ($request->input('type') === 'pdf') {
                $pdf = PDF::loadView('iktisar.rektor.iktisarBulananKaUnit.raportPdf', compact('data', 'tanggalInput'))->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'potrait');
                return $pdf->download('raport_' . $data->name . '_' . $tanggalInput->format('Y-m-d') . '.pdf');
            } else {
                return view('iktisar.rektor.iktisarBulananKaUnit.cekraport', compact('data', 'tanggalInput'));
            }
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
    ...
    public function rekap()
    {
        ...
        return view('iktisar.rektor.iktisarBulananKaUnit.rekap.index', compact('users'));
    }
    ...
    public function reportRaport(Request $request)
    {
        ...
        $data = DB::table('users')
            ...
            ->select(
                ...
                'iktisar_rektor_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'iktisar_rektor_bulanan_perilaku.total_nilai_presentase',
                'iktisar_rektor_bulanan_perilaku.created_insert'
            )
            ->where('iktisar_rektor_bulanan_perilaku.user_id', $id)
            ->orderBy('iktisar_rektor_bulanan_perilaku.created_insert', 'desc') // Urutkan dari terbaru ke lama
            ->get(); // Menggunakan get() agar mendapatkan semua data
        // dd($data);

        if ($data->isNotEmpty()) {
            return view('iktisar.rektor.iktisarBulananKaUnit.rekap.report', compact('data'));
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
        }
    ...
    {
        // Ambil data dari tabel iktisar_rektor_bulanan_perilaku
        $iktisarStaffBulananPerilaku = DB::table('iktisar_rektor_bulanan_perilaku')
        ...
        // Ambil data dari tabel iktisar_rektor_bulanan_kompetensi berdasarkan id_staff_perilaku
        $iktisarStaffBulananKompetensi = DB::table('iktisar_rektor_bulanan_kompetensi')
            ->where('id_staff_perilaku', $iktisarStaffBulananPerilaku->id)
            ->get();

        // Kirim data ke view
        return view('iktisar.rektor.iktisarBulananKaUnit.rekap.show', compact('iktisarStaffBulananPerilaku', 'iktisarStaffBulananKompetensi'));
    }
}
