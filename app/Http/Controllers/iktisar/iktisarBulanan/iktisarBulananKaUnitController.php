...
use App\Models\iktisar\kaunit\iktisarBulananKaunitKompetensi;
use App\Models\iktisar\kaunit\iktisarBulananKaunitPerilaku;
use App\Models\User;
...
class iktisarBulananKaUnitController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarBulananKaunitPerilaku::create($data1);
            ...
            function saveData($jenisPekerjaan, $jumlahBobot, $question, $id1)
            {
                foreach ($jenisPekerjaan as $key => $value) {
                    ...
                    iktisarBulananKaunitKompetensi::create($data2);
                }
            }
    ...
    public function edit(Request $request)
    {
        ...
        if ($iktisarStaffBulananPerilaku) {
            // Retrieve the corresponding rows from the iktisar_kaunit_bulanan_kompetensi table
        ...
        return view('iktisar.iktisarBulananKaUnit.edit', [
            'users' => $users,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarBulananKaunitPerilaku::find($id);
            ...
            $existingJenisPekerjaan = iktisarBulananKaunitKompetensi::where('id_staff_perilaku', $id)->pluck('jenis_pekerjaan')->toArray();

            // Loop melalui jenis pekerjaan yang diterima dari form
            foreach ($jenisPekerjaan as $key => $value) {
                $iktisarStaffBulananKompetensi = iktisarBulananKaunitKompetensi::where('id_staff_perilaku', $id)
                ...
                if (!$iktisarStaffBulananKompetensi) {
                    ...
                    iktisarBulananKaunitKompetensi::create($data);
                } else {
                    ...
                    iktisarBulananKaunitKompetensi::where('id', $iktisarStaffBulananKompetensi->id)->update($data);
                }
            }
            ...
            foreach ($existingJenisPekerjaan as $existingJenis) {
                ...
                if (!in_array($existingJenis, $jenisPekerjaan)) {
                    iktisarBulananKaunitKompetensi::where('id_staff_perilaku', $id)->where('jenis_pekerjaan', $existingJenis)->delete();
                }
            }
    ...
    public function raportStaff(Request $request, $user_id)
    {
        ...
        if ($request->filled('keyword')) {
            ...
            $users = $dataUser->whereExists(function ($query) use ($user_id, $date) {
                $query->select(DB::raw(1))
                    ->from('iktisar_kaunit_bulanan_perilaku')
                    ->whereRaw('users.id = iktisar_kaunit_bulanan_perilaku.user_id')
                    ->whereMonth('iktisar_kaunit_bulanan_perilaku.created_insert', $date->month)
                    ->whereYear('iktisar_kaunit_bulanan_perilaku.created_insert', $date->year)
                    ->where('iktisar_kaunit_bulanan_perilaku.user_id', $user_id);
            });
        } else {
            $users = $dataUser;
        }
        ...
        $data = $users
            ->leftJoin('iktisar_kaunit_bulanan_perilaku', 'users.id', '=', 'iktisar_kaunit_bulanan_perilaku.user_id')
            ->select(
                'users.*',
                'iktisar_kaunit_bulanan_perilaku.user_id',
                'iktisar_kaunit_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'iktisar_kaunit_bulanan_perilaku.total_nilai_presentase',
            )
            ->where('iktisar_kaunit_bulanan_perilaku.user_id', $user_id)
            ->whereMonth('iktisar_kaunit_bulanan_perilaku.created_insert', $date->month)
            ->whereYear('iktisar_kaunit_bulanan_perilaku.created_insert', $date->year)
        ...
        if (!empty($data->user_id)) {
            return view('iktisar.iktisarBulananKaUnit.raport', compact('data'));
        } else {
            toast('Data Empty :(', 'error');
            return redirect()->back();
        }
    ...
    {
        return view('iktisar.iktisarBulananKaUnit.searchdatapoin');
    }
    ...
    public function dataPoin(Request $request)
    {
        ...
        // Display the data on a form
        return view('iktisar.iktisarBulananKaUnit.detailPoin', [
            'users' => $users,
            'iktisarStaffBulananPerilaku' => $iktisarStaffBulananPerilaku,
            'iktisarStaffBulananKompetensi' => $iktisarStaffBulananKompetensi
        ]);
    ...
    public function searchRaportIktisar()
    {
        ...
        return view('iktisar.iktisarBulananKaUnit.searchdataraport', compact('users'));
    }
    ...
    //         ->whereYear('iktisar_kaunit_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->whereMonth('iktisar_kaunit_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->first();

    //     // dd($data);
    //     if (!empty($data)) {
    //         return view('iktisar.iktisarBulananKaUnit.cekraport', compact('data'));
    //     } else {
    //         toast('Data Empty', 'error');
    //         return redirect()->back();
    //     }
    ...
    public function staffRaportIktisar(Request $request)
    {
        ...
        if (!empty($data)) {
            if ($request->input('type') === 'pdf') {
                $pdf = PDF::loadView('iktisar.iktisarBulananKaUnit.raportPdf', compact('data', 'tanggalInput'))->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'potrait');
                return $pdf->download('raport_' . $data->name . '_' . $tanggalInput->format('Y-m-d') . '.pdf');
            } else {
                return view('iktisar.iktisarBulananKaUnit.cekraport', compact('data', 'tanggalInput'));
            }
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
    ...
    public function rekap()
    {
        ...
        return view('iktisar.iktisarBulananKaUnit.rekap.index', compact('users'));
    }
    ...
    public function reportRaport(Request $request)
    {
        ...
        if ($data->isNotEmpty()) {
            return view('iktisar.iktisarBulananKaUnit.rekap.report', compact('data'));
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
        }
