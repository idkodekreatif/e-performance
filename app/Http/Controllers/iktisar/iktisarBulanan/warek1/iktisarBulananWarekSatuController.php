...
namespace App\Http\Controllers\iktisar\iktisarBulanan\warek1;

use App\Http\Controllers\Controller;
use App\Models\iktisar\kaunit\warek1\iktisarWarek1BulananKompetensi;
use App\Models\iktisar\kaunit\warek1\iktisarWarek1BulananPerilaku;
use App\Models\User;
use Carbon\Carbon;
...
class iktisarBulananWarekSatuController extends Controller
{
    // penilai
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarWarek1BulananPerilaku::create($data1);
            ...
            function saveData($jenisPekerjaan, $jumlahBobot, $question, $id1)
            {
                foreach ($jenisPekerjaan as $key => $value) {
                    ...
                    iktisarWarek1BulananKompetensi::create($data2);
                }
            }
    ...
    public function edit(Request $request)
    {
        ...
        if ($iktisarStaffBulananPerilaku) {
            // Retrieve the corresponding rows from the iktisar_warek1_bulanan_kompetensi table
        ...
        return view('iktisar.warek1.iktisarBulananKaUnit.edit', [
            'users' => $users,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarWarek1BulananPerilaku::find($id);
            ...
            $existingJenisPekerjaan = iktisarWarek1BulananKompetensi::where('id_staff_perilaku', $id)->pluck('jenis_pekerjaan')->toArray();

            // Loop melalui jenis pekerjaan yang diterima dari form
            foreach ($jenisPekerjaan as $key => $value) {
                $iktisarStaffBulananKompetensi = iktisarWarek1BulananKompetensi::where('id_staff_perilaku', $id)
                ...
                if (!$iktisarStaffBulananKompetensi) {
                    ...
                    iktisarWarek1BulananKompetensi::create($data);
                } else {
                    ...
                    iktisarWarek1BulananKompetensi::where('id', $iktisarStaffBulananKompetensi->id)->update($data);
                }
            }
            ...
            foreach ($existingJenisPekerjaan as $existingJenis) {
                ...
                if (!in_array($existingJenis, $jenisPekerjaan)) {
                    iktisarWarek1BulananKompetensi::where('id_staff_perilaku', $id)->where('jenis_pekerjaan', $existingJenis)->delete();
                }
            }
    ...
    public function raportStaff(Request $request, $user_id)
    {
        ...
        if ($request->filled('keyword')) {
            ...
            $users = $dataUser->whereExists(function ($query) use ($user_id, $date) {
                $query->select(DB::raw(1))
                    ->from('iktisar_warek1_bulanan_perilaku')
                    ->whereRaw('users.id = iktisar_warek1_bulanan_perilaku.user_id')
                    ->whereMonth('iktisar_warek1_bulanan_perilaku.created_insert', $date->month)
                    ->whereYear('iktisar_warek1_bulanan_perilaku.created_insert', $date->year)
                    ->where('iktisar_warek1_bulanan_perilaku.user_id', $user_id);
            });
        } else {
            $users = $dataUser;
        }
        ...
        if (!empty($data->user_id)) {
            return view('iktisar.warek1.iktisarBulananKaUnit.raport', compact('data'));
        } else {
            toast('Data Empty :(', 'error');
            return redirect()->back();
        }
    ...
    {
        return view('iktisar.warek1.iktisarBulananKaUnit.searchdatapoin');
    }
    ...
    public function dataPoin(Request $request)
    {
        ...
        // Display the data on a form
        return view('iktisar.warek1.iktisarBulananKaUnit.detailPoin', [
            'users' => $users,
            'iktisarStaffBulananPerilaku' => $iktisarStaffBulananPerilaku,
            'iktisarStaffBulananKompetensi' => $iktisarStaffBulananKompetensi
        ]);
    ...
    public function staffRaportIktisar(Request $request)
    {
        ...
        if (!empty($data)) {
            if ($request->input('type') === 'pdf') {
                $pdf = PDF::loadView('iktisar.warek1.iktisarBulananKaUnit.raportPdf', compact('data', 'tanggalInput'))->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'potrait');
                return $pdf->download('raport_' . $data->name . '_' . $tanggalInput->format('Y-m-d') . '.pdf');
            } else {
                return view('iktisar.warek1.iktisarBulananKaUnit.cekraport', compact('data', 'tanggalInput'));
            }
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
