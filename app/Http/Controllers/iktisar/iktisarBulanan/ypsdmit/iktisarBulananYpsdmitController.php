...
namespace App\Http\Controllers\iktisar\iktisarBulanan\ypsdmit;

use App\Http\Controllers\Controller;
use App\Models\iktisar\kaunit\ypsdmit\iktisarYpsdmitBulananKaunitKompetensi;
use App\Models\iktisar\kaunit\ypsdmit\iktisarYpsdmitBulananKaunitPerilaku;
use App\Models\User;
use Carbon\Carbon;
...
class iktisarBulananYpsdmitController extends Controller
{
    public function create()
    {
        ...
        return view('iktisar.ypsdmit.iktisarBulananKaUnit.create', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarYpsdmitBulananKaunitPerilaku::create($data1);
            ...
            function saveData($jenisPekerjaan, $jumlahBobot, $question, $id1)
            {
                foreach ($jenisPekerjaan as $key => $value) {
                    ...
                    iktisarYpsdmitBulananKaunitKompetensi::create($data2);
                }
            }
    ...
    public function searchDataEdit()
    {
        ...
        return view('iktisar.ypsdmit.iktisarBulananKaUnit.searchdata', compact('users'));
    }
    // penilai
    public function edit(Request $request)
    {
        ...
        $iktisarStaffBulananPerilaku = DB::table('kaunit_ypsdmit_bulanan_perilaku')
        ...
        if ($iktisarStaffBulananPerilaku) {
            // Retrieve the corresponding rows from the kaunit_ypsdmit_bulanan_kompetensi table
            $iktisarStaffBulananKompetensi = DB::table('kaunit_ypsdmit_bulanan_kompetensi')
        ...
        return view('iktisar.ypsdmit.iktisarBulananKaUnit.edit', [
            'users' => $users,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarYpsdmitBulananKaunitPerilaku::find($id);
            ...
            $existingJenisPekerjaan = iktisarYpsdmitBulananKaunitKompetensi::where('id_staff_perilaku', $id)->pluck('jenis_pekerjaan')->toArray();

            // Loop melalui jenis pekerjaan yang diterima dari form
            foreach ($jenisPekerjaan as $key => $value) {
                $iktisarStaffBulananKompetensi = iktisarYpsdmitBulananKaunitKompetensi::where('id_staff_perilaku', $id)
                ...
                if (!$iktisarStaffBulananKompetensi) {
                    ...
                    iktisarYpsdmitBulananKaunitKompetensi::create($data);
                } else {
                    ...
                    iktisarYpsdmitBulananKaunitKompetensi::where('id', $iktisarStaffBulananKompetensi->id)->update($data);
                }
            }
            ...
            foreach ($existingJenisPekerjaan as $existingJenis) {
                ...
                if (!in_array($existingJenis, $jenisPekerjaan)) {
                    iktisarYpsdmitBulananKaunitKompetensi::where('id_staff_perilaku', $id)->where('jenis_pekerjaan', $existingJenis)->delete();
                }
            }
    ...
    public function raportStaff(Request $request, $user_id)
    {
        ...
        if ($request->filled('keyword')) {
            ...
            $users = $dataUser->whereExists(function ($query) use ($user_id, $date) {
                $query->select(DB::raw(1))
                    ->from('kaunit_ypsdmit_bulanan_perilaku')
                    ->whereRaw('users.id = kaunit_ypsdmit_bulanan_perilaku.user_id')
                    ->whereMonth('kaunit_ypsdmit_bulanan_perilaku.created_insert', $date->month)
                    ->whereYear('kaunit_ypsdmit_bulanan_perilaku.created_insert', $date->year)
                    ->where('kaunit_ypsdmit_bulanan_perilaku.user_id', $user_id);
            });
        } else {
            $users = $dataUser;
        }
        ...
        $data = $users
            ->leftJoin('kaunit_ypsdmit_bulanan_perilaku', 'users.id', '=', 'kaunit_ypsdmit_bulanan_perilaku.user_id')
            ->select(
                'users.*',
                'kaunit_ypsdmit_bulanan_perilaku.user_id',
                'kaunit_ypsdmit_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'kaunit_ypsdmit_bulanan_perilaku.total_nilai_presentase',
            )
            ->where('kaunit_ypsdmit_bulanan_perilaku.user_id', $user_id)
            ->whereMonth('kaunit_ypsdmit_bulanan_perilaku.created_insert', $date->month)
            ->whereYear('kaunit_ypsdmit_bulanan_perilaku.created_insert', $date->year)
        ...
        if (!empty($data->user_id)) {
            return view('iktisar.ypsdmit.iktisarBulananKaUnit.raport', compact('data'));
        } else {
            toast('Data Empty :(', 'error');
            return redirect()->back();
        }
    ...
    {
        return view('iktisar.ypsdmit.iktisarBulananKaUnit.searchdatapoin');
    }
    ...
    public function dataPoin(Request $request)
    {
        ...
        // Display the data on a form
        return view('iktisar.ypsdmit.iktisarBulananKaUnit.detailPoin', [
            'users' => $users,
            'iktisarStaffBulananPerilaku' => $iktisarStaffBulananPerilaku,
            'iktisarStaffBulananKompetensi' => $iktisarStaffBulananKompetensi
        ]);
    ...
    public function searchRaportIktisar()
    {
        ...
        return view('iktisar.ypsdmit.iktisarBulananKaUnit.searchdataraport', compact('users'));
    }
    ...
    //             'kaunit_ypsdmit_bulanan_perilaku.total_nilai_presentase',
    //         )
    //         ->where('kaunit_ypsdmit_bulanan_perilaku.user_id', $id)
    //         ->whereYear('kaunit_ypsdmit_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->whereMonth('kaunit_ypsdmit_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->first();

    //     // dd($data);
    //     if (!empty($data)) {
    //         return view('iktisar.ypsdmit.iktisarBulananKaUnit.cekraport', compact('data'));
    //     } else {
    //         toast('Data Empty', 'error');
    //         return redirect()->back();
    //     }
    ...
    public function staffRaportIktisar(Request $request)
    {
        ...
        if (!empty($data)) {
            if ($request->input('type') === 'pdf') {
                $pdf = PDF::loadView('iktisar.ypsdmit.iktisarBulananKaUnit.raportPdf', compact('data', 'tanggalInput'))->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'potrait');
                return $pdf->download('raport_' . $data->name . '_' . $tanggalInput->format('Y-m-d') . '.pdf');
            } else {
                return view('iktisar.ypsdmit.iktisarBulananKaUnit.cekraport', compact('data', 'tanggalInput'));
            }
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
