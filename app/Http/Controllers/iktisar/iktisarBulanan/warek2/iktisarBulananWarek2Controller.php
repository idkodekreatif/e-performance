...
namespace App\Http\Controllers\iktisar\iktisarBulanan\warek2;

use App\Http\Controllers\Controller;
use App\Models\iktisar\kaunit\warek2\iktisarWarek2BulananKompetensi;
use App\Models\iktisar\kaunit\warek2\iktisarWarek2BulananPerilaku;
use App\Models\User;
use Carbon\Carbon;
...
class iktisarBulananWarek2Controller extends Controller
{
    // penilai
    public function create()
    {
        ...
        return view('iktisar.warek2.iktisarBulananKaUnit.create', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarWarek2BulananPerilaku::create($data1);
            ...
            function saveData($jenisPekerjaan, $jumlahBobot, $question, $id1)
            {
                foreach ($jenisPekerjaan as $key => $value) {
                    ...
                    iktisarWarek2BulananKompetensi::create($data2);
                }
            }
    ...
    public function searchDataEdit()
    {
        ...
        return view('iktisar.warek2.iktisarBulananKaUnit.searchdata', compact('users'));
    }
    // penilai
    public function edit(Request $request)
    {
        ...
        $iktisarStaffBulananPerilaku = DB::table('iktisar_warek2_bulanan_perilaku')
        ...
        if ($iktisarStaffBulananPerilaku) {
            // Retrieve the corresponding rows from the iktisar_warek2_bulanan_kompetensi table
            $iktisarStaffBulananKompetensi = DB::table('iktisar_warek2_bulanan_kompetensi')
        ...
        return view('iktisar.warek2.iktisarBulananKaUnit.edit', [
            'users' => $users,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarWarek2BulananPerilaku::find($id);
            ...
            $existingJenisPekerjaan = iktisarWarek2BulananKompetensi::where('id_staff_perilaku', $id)->pluck('jenis_pekerjaan')->toArray();

            // Loop melalui jenis pekerjaan yang diterima dari form
            foreach ($jenisPekerjaan as $key => $value) {
                $iktisarStaffBulananKompetensi = iktisarWarek2BulananKompetensi::where('id_staff_perilaku', $id)
                ...
                if (!$iktisarStaffBulananKompetensi) {
                    ...
                    iktisarWarek2BulananKompetensi::create($data);
                } else {
                    ...
                    iktisarWarek2BulananKompetensi::where('id', $iktisarStaffBulananKompetensi->id)->update($data);
                }
            }
            ...
            foreach ($existingJenisPekerjaan as $existingJenis) {
                ...
                if (!in_array($existingJenis, $jenisPekerjaan)) {
                    iktisarWarek2BulananKompetensi::where('id_staff_perilaku', $id)->where('jenis_pekerjaan', $existingJenis)->delete();
                }
            }
    ...
    public function raportStaff(Request $request, $user_id)
    {
        ...
        if ($request->filled('keyword')) {
            ...
            $users = $dataUser->whereExists(function ($query) use ($user_id, $date) {
                $query->select(DB::raw(1))
                    ->from('iktisar_warek2_bulanan_perilaku')
                    ->whereRaw('users.id = iktisar_warek2_bulanan_perilaku.user_id')
                    ->whereMonth('iktisar_warek2_bulanan_perilaku.created_insert', $date->month)
                    ->whereYear('iktisar_warek2_bulanan_perilaku.created_insert', $date->year)
                    ->where('iktisar_warek2_bulanan_perilaku.user_id', $user_id);
            });
        } else {
            $users = $dataUser;
        }
        ...
        $data = $users
            ->leftJoin('iktisar_warek2_bulanan_perilaku', 'users.id', '=', 'iktisar_warek2_bulanan_perilaku.user_id')
            ->select(
                'users.*',
                'iktisar_warek2_bulanan_perilaku.user_id',
                'iktisar_warek2_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'iktisar_warek2_bulanan_perilaku.total_nilai_presentase',
            )
            ->where('iktisar_warek2_bulanan_perilaku.user_id', $user_id)
            ->whereMonth('iktisar_warek2_bulanan_perilaku.created_insert', $date->month)
            ->whereYear('iktisar_warek2_bulanan_perilaku.created_insert', $date->year)
        ...
        if (!empty($data->user_id)) {
            return view('iktisar.warek2.iktisarBulananKaUnit.raport', compact('data'));
        } else {
            toast('Data Empty :(', 'error');
            return redirect()->back();
        }
    ...
    {
        return view('iktisar.warek2.iktisarBulananKaUnit.searchdatapoin');
    }
    ...
    public function dataPoin(Request $request)
    {
        ...
        // Display the data on a form
        return view('iktisar.warek2.iktisarBulananKaUnit.detailPoin', [
            'users' => $users,
            'iktisarStaffBulananPerilaku' => $iktisarStaffBulananPerilaku,
            'iktisarStaffBulananKompetensi' => $iktisarStaffBulananKompetensi
        ]);
    ...
    public function searchRaportIktisar()
    {
        ...
        return view('iktisar.warek2.iktisarBulananKaUnit.searchdataraport', compact('users'));
    }
    ...
    public function staffRaportIktisar(Request $request)
    {
        ...
        $data = DB::table('users')
            ...
            ->where('iktisar_warek2_bulanan_perilaku.user_id', $id)
            ->whereYear('iktisar_warek2_bulanan_perilaku.created_insert', $tanggalInput)
            ->whereMonth('iktisar_warek2_bulanan_perilaku.created_insert', $tanggalInput)
            ->first();

        if (!empty($data)) {
            if ($request->input('type') === 'pdf') {
                $pdf = PDF::loadView('iktisar.warek2.iktisarBulananKaUnit.raportPdf', compact('data', 'tanggalInput'))->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'potrait');
                return $pdf->download('raport_' . $data->name . '_' . $tanggalInput->format('Y-m-d') . '.pdf');
            } else {
                return view('iktisar.warek2.iktisarBulananKaUnit.cekraport', compact('data', 'tanggalInput'));
            }
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
    ...
    public function rekap()
    {
        ...
        return view('iktisar.warek2.iktisarBulananKaUnit.rekap.index', compact('users'));
    }
    ...
    public function reportRaport(Request $request)
    {
        ...
        $data = DB::table('users')
            ...
            ->select(
                ...
                'iktisar_warek2_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'iktisar_warek2_bulanan_perilaku.total_nilai_presentase',
                'iktisar_warek2_bulanan_perilaku.created_insert'
            )
            ->where('iktisar_warek2_bulanan_perilaku.user_id', $id)
            ->orderBy('iktisar_warek2_bulanan_perilaku.created_insert', 'desc') // Urutkan dari terbaru ke lama
            ->get(); // Menggunakan get() agar mendapatkan semua data
        // dd($data);

        if ($data->isNotEmpty()) {
            return view('iktisar.warek2.iktisarBulananKaUnit.rekap.report', compact('data'));
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
        }
    ...
    {
        // Ambil data dari tabel iktisar_warek2_bulanan_perilaku
        $iktisarStaffBulananPerilaku = DB::table('iktisar_warek2_bulanan_perilaku')
        ...
        // Ambil data dari tabel iktisar_warek2_bulanan_kompetensi berdasarkan id_staff_perilaku
        $iktisarStaffBulananKompetensi = DB::table('iktisar_warek2_bulanan_kompetensi')
            ->where('id_staff_perilaku', $iktisarStaffBulananPerilaku->id)
            ->get();

        // Kirim data ke view
        return view('iktisar.warek2.iktisarBulananKaUnit.rekap.show', compact('iktisarStaffBulananPerilaku', 'iktisarStaffBulananKompetensi'));
    }
}
