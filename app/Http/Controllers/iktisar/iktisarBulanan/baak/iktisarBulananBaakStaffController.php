...
use App\Models\iktisar\staff\baak\iktisarBaakBulananStaffKompetensi;
use App\Models\iktisar\staff\baak\iktisarBaakBulananStaffPerilaku;
use App\Models\User;
use Carbon\Carbon;
...
class iktisarBulananBaakStaffController extends Controller
{
    // penilai
    public function create()
    {
        ...
        return view('iktisar.baak.iktisarBulananStaff.create', compact('users'));
    }
    // preses penilai
    public function store(Request $request)
    {
        DB::beginTransaction();
        try {
            // Insert data into iktisar_staff_baak_bulanan_perilaku table
            $data1 = [
            ...
            // Insert the data into the iktisar_staff_baak_bulanan_perilaku table and get the inserted row ID
            $iktisarStaffBulananPerilaku = iktisarBaakBulananStaffPerilaku::create($data1);
            $id1 = $iktisarStaffBulananPerilaku->id;

            // Insert data into iktisar_staff_baak_bulanan_kompetensi table for each job type in the jenisPekerjaan array
            function saveData($jenisPekerjaan, $jumlahBobot, $question, $id1)
            {
                foreach ($jenisPekerjaan as $key => $value) {
                    ...
                    iktisarBaakBulananStaffKompetensi::create($data2);
                }
            }
    ...
    public function searchDataEdit()
    {
        ...
        return view('iktisar.baak.iktisarBulananStaff.searchdata', compact('users'));
    }
    // penilai
    public function edit(Request $request)
    {
        ...
        $iktisarStaffBulananPerilaku = DB::table('iktisar_staff_baak_bulanan_perilaku')
        ...
        if ($iktisarStaffBulananPerilaku) {
            // Retrieve the corresponding rows from the iktisar_staff_baak_bulanan_kompetensi table
            $iktisarStaffBulananKompetensi = DB::table('iktisar_staff_baak_bulanan_kompetensi')
        ...
        return view('iktisar.baak.iktisarBulananStaff.edit', [
            'users' => $users,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $iktisarStaffBulananPerilaku = iktisarBaakBulananStaffPerilaku::find($id);
            ...
            $existingJenisPekerjaan = iktisarBaakBulananStaffKompetensi::where('id_staff_perilaku', $id)->pluck('jenis_pekerjaan')->toArray();

            // Loop melalui jenis pekerjaan yang diterima dari form
            foreach ($jenisPekerjaan as $key => $value) {
                $iktisarStaffBulananKompetensi = iktisarBaakBulananStaffKompetensi::where('id_staff_perilaku', $id)
                ...
                if (!$iktisarStaffBulananKompetensi) {
                    ...
                    iktisarBaakBulananStaffKompetensi::create($data);
                } else {
                    ...
                    iktisarBaakBulananStaffKompetensi::where('id', $iktisarStaffBulananKompetensi->id)->update($data);
                }
            }
            ...
            foreach ($existingJenisPekerjaan as $existingJenis) {
                ...
                if (!in_array($existingJenis, $jenisPekerjaan)) {
                    iktisarBaakBulananStaffKompetensi::where('id_staff_perilaku', $id)->where('jenis_pekerjaan', $existingJenis)->delete();
                }
            }
    ...
    public function raportStaff(Request $request, $user_id)
    {
        ...
        if ($request->filled('keyword')) {
            ...
            $users = $dataUser->whereExists(function ($query) use ($user_id, $date) {
                $query->select(DB::raw(1))
                    ->from('iktisar_staff_baak_bulanan_perilaku')
                    ->whereRaw('users.id = iktisar_staff_baak_bulanan_perilaku.user_id')
                    ->whereMonth('iktisar_staff_baak_bulanan_perilaku.created_insert', $date->month)
                    ->whereYear('iktisar_staff_baak_bulanan_perilaku.created_insert', $date->year)
                    ->where('iktisar_staff_baak_bulanan_perilaku.user_id', $user_id);
            });
        } else {
            $users = $dataUser;
        }
        ...
        $data = $users
            ->leftJoin('iktisar_staff_baak_bulanan_perilaku', 'users.id', '=', 'iktisar_staff_baak_bulanan_perilaku.user_id')
            ->select(
                'users.*',
                'iktisar_staff_baak_bulanan_perilaku.user_id',
                'iktisar_staff_baak_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'iktisar_staff_baak_bulanan_perilaku.total_nilai_presentase',
            )
            ->where('iktisar_staff_baak_bulanan_perilaku.user_id', $user_id)
            ->whereMonth('iktisar_staff_baak_bulanan_perilaku.created_insert', $date->month)
            ->whereYear('iktisar_staff_baak_bulanan_perilaku.created_insert', $date->year)
        ...
        if (!empty($data->user_id)) {
            return view('iktisar.baak.iktisarBulananStaff.raport', compact('data'));
        } else {
            toast('Data Empty :(', 'error');
            return redirect()->back();
        }
    ...
    {
        return view('iktisar.baak.iktisarBulananStaff.searchdatapoin');
    }
    ...
    public function dataPoin(Request $request)
    {
        ...
        // Display the data on a form
        return view('iktisar.baak.iktisarBulananStaff.detailPoin', [
            'users' => $users,
            'iktisarStaffBulananPerilaku' => $iktisarStaffBulananPerilaku,
            'iktisarStaffBulananKompetensi' => $iktisarStaffBulananKompetensi
        ]);
    ...
    public function searchRaportIktisar()
    {
        ...
        return view('iktisar.baak.iktisarBulananStaff.searchdataraport', compact('users'));
    }
    ...
    //             'iktisar_staff_baak_bulanan_perilaku.total_nilai_presentase',
    //         )
    //         ->where('iktisar_staff_baak_bulanan_perilaku.user_id', $id)
    //         ->whereYear('iktisar_staff_baak_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->whereMonth('iktisar_staff_baak_bulanan_perilaku.created_insert', $tanggalInput)
    //         ->first();

    //     // dd($data);
    //     if (!empty($data)) {
    //         return view('iktisar.baak.iktisarBulananStaff.cekraport', compact('data'));
    //     } else {
    //         toast('Data Empty', 'error');
    //         return redirect()->back();
    //     }
    ...
    public function staffRaportIktisar(Request $request)
    {
        ...
        if (!empty($data)) {
            if ($request->input('type') === 'pdf') {
                $pdf = PDF::loadView('iktisar.baak.iktisarBulananStaff.raportPdf', compact('data', 'tanggalInput'))->setOptions(['defaultFont' => 'sans-serif'])->setPaper('A4', 'potrait');
                return $pdf->download('raport_' . $data->name . '_' . $tanggalInput->format('Y-m-d') . '.pdf');
            } else {
                return view('iktisar.baak.iktisarBulananStaff.cekraport', compact('data', 'tanggalInput'));
            }
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
