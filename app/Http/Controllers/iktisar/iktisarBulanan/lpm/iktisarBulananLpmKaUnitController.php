...
class iktisarBulananLpmKaUnitController extends Controller
{
    ...
    public function rekap()
    {
        ...
        return view('iktisar.lpm.iktisarBulananKaUnit.rekap.index', compact('users'));
    }
    ...
    public function reportRaport(Request $request)
    {
        ...
        $data = DB::table('users')
            ...
            ->select(
                ...
                'iktisar_lpm_bulanan_perilaku.output_total_sementara_kinerja_perilaku',
                'iktisar_lpm_bulanan_perilaku.total_nilai_presentase',
                'iktisar_lpm_bulanan_perilaku.created_insert'
            )
            ->where('iktisar_lpm_bulanan_perilaku.user_id', $id)
            ->orderBy('iktisar_lpm_bulanan_perilaku.created_insert', 'desc') // Urutkan dari terbaru ke lama
            ->get(); // Menggunakan get() agar mendapatkan semua data
        // dd($data);

        if ($data->isNotEmpty()) {
            return view('iktisar.lpm.iktisarBulananKaUnit.rekap.report', compact('data'));
        } else {
            toast('Data Empty', 'error');
            return redirect()->back();
        }
    ...
    {
        // Ambil data dari tabel iktisar_lpm_bulanan_perilaku
        $iktisarStaffBulananPerilaku = DB::table('iktisar_lpm_bulanan_perilaku')
        ...
        // Ambil data dari tabel iktisar_lpm_bulanan_kompetensi berdasarkan id_staff_perilaku
        $iktisarStaffBulananKompetensi = DB::table('iktisar_lpm_bulanan_kompetensi')
            ->where('id_staff_perilaku', $iktisarStaffBulananPerilaku->id)
            ->get();

        // Kirim data ke view
        return view('iktisar.lpm.iktisarBulananKaUnit.rekap.show', compact('iktisarStaffBulananPerilaku', 'iktisarStaffBulananKompetensi'));
    }
}
